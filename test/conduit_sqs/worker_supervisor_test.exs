defmodule ConduitSQS.WorkerSupervisorTest do
  ...
  describe "init/1" do
    test "returns child specs equal to worker pool size" do
      assert {:ok, {sup_opts, child_specs}} = WorkerSupervisor.init([Broker, :test, Subscriber, [], [worker_pool_size: 2]])

      assert sup_opts == {:one_for_one, 3, 5}

      assert child_specs == [
        {{ConduitSQS.Worker, 1},
          {ConduitSQS.Worker, :start_link, [Broker, :test, Subscriber, [], [worker_pool_size: 2]]},
